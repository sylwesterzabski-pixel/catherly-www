...
0d. **OTWARTE PO `WWW/090` KROK 3 v2 (ADR-064, 2026-09-06) — pięć decyzji
    właściciela, wszystkie zgłoszone z liczbami.**
   a. **TON DOMYŚLNY STRONY** (T68) — wzorzec 70,9 % jasnych pikseli,
      my 47,4 %. Odwrócenie to zmiana palety: dziesięć ról korpusu
      ciemnego plus odwrócenie sensu `data-ton` (dziś wyjątkiem jest
      jasny, po odwróceniu byłby ciemny). **Największa pojedyncza
      różnica wobec wzorca.**
   b. **Treść dla sekcji „pamięć" i „wzrost"** — nagłówków nie ma
      w `content/`, a implementacja treści nie pisze. Bez nich te dwie
      sekcje nie mogą powstać.
   c. **Ekrany w ramach urządzeń** — „zrzuty fali 1" nie istnieją (fala 1
      to fotografie). Wstawienie zrzutów aplikacji odwraca decyzje
      `WWW/072` i `WWW/083` („zero zrzutów aplikacji i mockupów").
   d. **Trzy karty filarów czy cztery** — wzorzec pokazuje trzy, mamy
      cztery; zdjęcie „Wyników" jest decyzją o treści.
   e. **Zdjęcie czterech sekcji z głównej** — obawy/FAQ są związane
      decyzją O-7 („jednym pakietem albo wcale") i pilnowane przez
      `toHaveCount(6)`.

0c. **OTWARTE PO `WWW/090` (ADR-065, 2026-09-06).**
   a. **KROK 3 — przebudowa strony głównej na 10 sekcji wzorca.** Pomiar
      gotowy i kompletny (ADR-065 §2–§4), wzorzec w repozytorium z sumą
      kontrolną, punkt odniesienia nakładki zmierzony (27,7 % na pasie
      górnym przy 1440). Do zrobienia w całości, nie w częściach.
   b. **`grafit-klamr` bez konsumenta** — rola weszła pod krok 3.
      Jeśli krok 3 nie wejdzie, rola schodzi razem z nim.
   c. **Skala nagłówków** (T67) — stosunek H2 : H1 wzorca 0,80 wobec
      naszego 0,50; wymaga przeliczenia całej skali z ADR-044, nie
      podmiany dwóch liczb.
   d. **Zrzut wzorca przy znanej szerokości okna** (T66) — bez niego
      rozmiary pisma pozostają nasze, bo kolaż ich nie rozstrzyga.

0b. **OTWARTE PO `WWW/089` (ADR-063, 2026-09-05) — trzy pozycje.**
   a. **Zakres parallaxu hero.** Wzorzec go NIE MA (T63), więc 3% jest
      wyborem wykonawcy pod poleceniem zlecenia. Do rozstrzygnięcia: czy
      zostaje jako decyzja własna, czy schodzi razem z tokenem.
   b. **Która sekcja przechodzi na układ bento** (T64). Pomiar gotowy
      i kompletny; brakuje przedmiotu. Kandydaci: karty funkcji, pas
      możliwości, sekcja kierunku.
   c. **Wejście „słowo po słowie"** (T65) — wymaga najpierw rozstrzygnięcia,
      czy dzieli nagłówek KOMPONENT czy ŹRÓDŁO TREŚCI, a dopiero potem
      pomiaru łamania w trzech językach na 390 i 320.

0. **OTWARTE PO `WWW/088` (ADR-062, 2026-09-05) — cztery pozycje, wszystkie
   z liczbą przy sobie.**
   a. **Pigułka: 50 px czy 24 px.** Zlecenie mapowało „sekcja/pigułka → 24”,
      **nie wykonałem tego i powstrzymuje to sam przeszczepiany pomiar**:
      najczęstszym promieniem React Bits jest przycięte `9999px`, czyli wzorzec
      trzyma pigułkę **obok** drabiny 12·16·24·32, nie zamiast niej. Zmiana
      dotknęłaby naraz paska nawigacji, przycisku hero, przycisku logowania
      i przełącznika okresu — dlatego stoi.
   b. **Czwarty szczebel promieni (32 px).** Nie wszedł, bo **nie ma
      konsumenta**: przejrzane 21 wystąpień `border-radius`, każde ma rolę na
      trzech niższych szczeblach. Wchodzi z pierwszym elementem, który go
      potrzebuje — albo nie wchodzi wcale.
   c. **Rola „akcent przygaszony”.** React Bits ją ma (`--card-secondary`,
      blada limonka na jasnym, ciemna oliwka na ciemnym), my nie. Nie tworzę
      roli bez konsumenta i bez pary kontrastowej — dołożenie podniosłoby
      `LICZBA_ROL` z 30, a ta liczba jest mechanizmem wymagającym ADR-a.
   d. **Tekst przygaszony i kreska z React Bits — odrzucone progiem.**
      `#737373` daje na naszym jasnym tle **4,35:1** przy progu 4,5 ze zlecenia;
      `#e5e5e5` daje na białej karcie **1,26:1** przy progu 1,30 z ADR-038.
      Gdyby miały wejść, musi wejść **razem z nimi decyzja o zejściu poniżej
      obu progów** — nie da się przyjąć tych wartości i zostawić progi.

0a. **`R-TYPO-03` — trzy drogi (T59).** Zlecenia wymieniają go jako warunek
   odbioru, a **w tym repozytorium nie istnieje** (0 trafień, kontrola
   pozytywna `R-AKCENT` 28). Albo odesłanie ma nieść treść z repozytorium,
   w którym żyje, albo strażnik ma tu powstać wraz z definicją „skrajnika”,
   albo identyfikator znika z warunków odbioru. **Do rozstrzygnięcia żadne
   okno nie może zaraportować, że jest zielony.**
...
- **SKOKOWE `scrollTo` NIE DAJE ANIMACJOM ANI JEDNEJ KLATKI POŚREDNIEJ —
  TRZY PODEJŚCIA Z RZĘDU DAŁY ZERO, KTÓRE BYŁO ZEREM NARZĘDZIA**
  (2026-09-05, `WWW/089` krok 1b). Sonda wejść przewijała `window.scrollTo`
  wprost do celu i próbkowała `opacity`/`transform` przez 1,6–2 s. Wynik: zero
  zmieniających się elementów na sześciu sekcjach, ~700 serii pomiarowych.
  Brzmiało jak ustalenie o wzorcu („React Bits nie animuje wejść"), a było
  ustaleniem o metodzie: przy skoku oś `view()` przeskakuje od razu na koniec
  zakresu, a element ląduje w stanie końcowym **przed pierwszą próbką**.
  **Rozstrzygnęła KONTROLA POZYTYWNA WSTRZYKNIĘTA W TEN SAM PRZEBIEG:**
  element z własną, znaną animacją **też nie zapalał**. Gdyby jej nie było,
  zero poszłoby do zwrotki jako własność wzorca. Przewijanie płynne, po ~24 px
  na klatkę, pokazało cztery rodziny wejść i pełne krzywe.
  **Praktycznie: sonda ruchu ma przewijać tak, jak przewija palec.**
- **KONTROLA POZYTYWNA, KTÓRA NIE MOŻE ZADZIAŁAĆ W TYM UKŁADZIE, MILCZY
  IDENTYCZNIE JAK KONTROLA UDANA** (2026-09-05, `WWW/089` krok 2). Sonda
  nachodzenia tekstu na kadr hero miała dowieść, że umie wykryć nachodzenie.
  Podejście 1: przesuń `h1` przez `transform` — **animacja wejścia bije styl
  wbudowany**, więc nic się nie ruszyło. Podejście 2: przesuń `h1` marginesem
  — margines spycha kadr o tyle samo, bo **oba są w jednym przepływie**.
  Dopiero podejście 3, ujemny margines KADRU, wciągnęło go pod tekst i
  kontrola zapaliła. **Wniosek mocniejszy od samego pomiaru:** te elementy
  nachodzić na siebie **nie mogą z konstrukcji**, a nie dlatego, że akurat
  starczyło miejsca. **Praktycznie: kontrola pozytywna musi umieć wywołać
  skutek W TYM UKŁADZIE, nie w wyobrażonym.**
- **MECHANIZM ZBUDOWANY DO POŁOWY ZAPALA BRAMKI Z POWODU NIEZWIĄZANEGO
  Z KODEM** (2026-09-05, `WWW/089` krok 0b). `WWW_DIST` weszło do
  `next.config.ts`, ale **cztery skrypty miały `.next` wpisane na sztywno**.
  Po pierwszym budowaniu pomiarowym „Linki" i „No-JS" zapaliły się na
  czerwono — a zmiana ich nie dotyczyła. To czerwień **myląca**: mówi
  o katalogu, a czyta się jako defekt kodu, więc kosztuje szukanie w złym
  miejscu. **Wykryte pomiarem, nie lekturą** — przy pisaniu konfiguracji nie
  przyszło mi do głowy, że bramki też czytają ten katalog.
  **Praktycznie: wprowadzając zmienną sterującą, policz WSZYSTKICH jej
  czytelników, nie tylko tego, dla którego ją wprowadzasz.**
- **ZNACZNIK HEREDOCU POSTAWIONY NA KOŃCU WIERSZA TREŚCI POŁYKA RESZTĘ
  POLECENIA** (2026-09-05). Blok `<<'KONIEC'` zakończony słowem `KONIEC`
  **w tej samej linii** co ostatnie zdanie tekstu nie kończy się wcale:
  powłoka wciąga dalszy ciąg polecenia do pliku. Objaw jest mylący —
  **polecenie kończy się bez żadnego wyjścia i bez błędu**, więc wygląda na
  wykonane. Wychwycone jednym sprawdzeniem (`git status` nie pokazał
  spodziewanej zmiany). **Praktycznie: po każdej edycji przez heredoc
  sprawdź `git diff --numstat` na pliku, który miał się zmienić.**

- **MUTACJA, KTÓRA SIĘ NIE WYKONAŁA, ZGŁOSIŁA SIĘ JAKO MUTACJA, KTÓRA NIE
  ZAPALIŁA** (2026-09-05, `WWW/088`). Skrypt dowodu mutacyjnego dla bramki
  parytetu próbował sparsować `content/en.json` — **a `content/en` jest
  KATALOGIEM**, nie plikiem. Python padł, łańcuch `&&` się urwał, zmienna
  z wynikiem została pusta, a gałąź `else` wypisała „### ZIELONA — MUTACJA NIE
  ZAPALIŁA ###”. **To jest najgorszy możliwy fałszywy wynik w tej rodzinie:**
  brzmi jak ustalenie o strażniku („nie widzi defektu”), a jest ustaleniem
  o moim skrypcie. Gdyby wpadł do zwrotki, zamknąłby pozycję nieistniejącym
  pomiarem. **Praktycznie: sonda mutacyjna ma DRUKOWAĆ, co zmutowała** — nazwę
  pliku i podmienioną wartość — a nie tylko werdykt po fakcie; brak wydruku
  mutacji znaczy, że mutacji nie było. To ta sama klasa co „test drukuje
  TOŻSAMOŚĆ przedmiotu, nie sam werdykt”, tylko po stronie mutacji.
  ⚠ **Wystąpiło DWA RAZY tego samego dnia**, drugi raz na tym samym pliku
  w innej postaci — pierwsze niepowodzenie nie nauczyło mnie sprawdzić, czym
  jest przedmiot, tylko poprawić parser.
- **PODMIANA CAŁEGO WIERSZA TAM, GDZIE MIAŁO BYĆ DOPISANIE NA POCZĄTKU**
  (2026-09-05, `WWW/088`). Aktualizując rozdz. 1 tego pliku, podmieniłem
  komórkę `STAN PRACY` **w całości** — i skasowałem tym łańcuch „Poprzednio…”,
  czyli **8 120 znaków historii** w dokumencie, którego jedynym zadaniem jest
  ciągłość. Wyszło natychmiast, bo nowa komórka była KRÓTSZA od starej, więc
  wydruk długości nie zgadzał się z zamiarem. **Praktycznie: przy każdej
  edycji tego pliku porównuj długość komórki przed i po — dopisanie ma
  WYDŁUŻAĆ.** Odtworzone z `git show HEAD:` i sklejone poprawnie; `git diff
  --numstat` po naprawie pokazuje 2 wiersze zmienione, oba dłuższe.
- **RÓWNOLEGŁE POMIARY WYDAJNOŚCI ZATRUWAJĄ SIĘ NAWZAJEM — i wiedziałem to
  ZANIM zmierzyłem, więc to nie jest pułapka, w którą wpadłem, tylko taka,
  którą warto zapisać** (2026-09-05, `WWW/088` krok 0.5). Lighthouse i tracing
  walczą o ten sam procesor; sześć przebiegów Lighthouse’a i dwa przebiegi
  tracingu poszły **szeregowo**, po zakończeniu wszystkiego, co działało w tle.
  Rozrzut LCP wyszedł 1,04× (R) i 1,00× (nasze) — czyli pomiar jest
  interpretowalny, co przy rozrzucie 1,67× z `WWW/077` nie było oczywiste.
- **STATYSTYKA ARKUSZA POLICZONA Z DEKLARACJI ZAMIAST Z WŁASNOŚCI WŁASNYCH**
  (2026-09-05, `WWW/088` krok 0.5). Pierwszy przebieg dał o wzorcu React Bits:
  „4 odstępy, 3 promienie, 1 rozmiar pisma” — nieprawdę, bo Tailwind v4 trzyma
  skalę w `--radius-*`, `--text-*`, `--leading-*`, a deklaracje tylko je
  składają. **Liczba wyglądała jak wynik i dała się zacytować.** Rozpoznane po
  tym, że była absurdalnie mała jak na stronę z czterema szczeblami promieni
  widocznymi gołym okiem w pomiarze DOM z `WWW/087-R`. **Praktycznie: zanim
  policzysz rozkład wartości w cudzym arkuszu, sprawdź, czy ten arkusz w ogóle
  trzyma wartości w deklaracjach.**
- **PODZLECENIE W TLE ZWRÓCIŁO ZERO WYNIKÓW I TRZEBA BYŁO ZROBIĆ ROBOTĘ
  SAMEMU** (2026-09-05, `WWW/088`). Rozpoznanie repozytorium puszczone
  równolegle jako pięć zadań padło w całości na błędach DNS do API — pięć
  z pięciu. **Nie ponawiałem**: odczyt repozytorium wykonany ręką jest tu
  pewniejszy niż drugie podejście do kanału, który właśnie zawiódł, a wynik
  i tak wymagał sprawdzenia. Zapisuję, bo w zwrotce trzeba było powiedzieć
  wprost, że rozpoznanie nie przyniosło nic — milczenie o tym wyglądałoby jak
...
| T53 | **sekcja opinii (2.7) ŚWIADOMIE NIE POWSTAŁA** — prawdziwych cytatów nie ma, a nasze odpowiedzi w tej roli byłyby pseudo-dowodem |
| T54 | **`pnpm install --ignore-scripts` nie ustawia `core.hooksPath`** — hooki nieaktywne, lintery nie biegną przed commitem |
| T55 | **model kosztu LCP jest nieaktualny** — elementem LCP na „/” jest dekoracja `aria-hidden` o kryciu 6%, nie zrzuty |
| T56 | **`margin-block-start` kropki nigdy się nie stosuje** — przykryty skrótem `margin` o wyższej swoistości, zmierzone 0 px |
| ~~T57~~ **ZAMKNIĘTE** | pełny zestaw e2e nie miał kadru szerokiego; zamknięte w ADR-056 trzecim i czwartym projektem |
| T58 | **rodzina ruchu „sekcja przy przewijaniu” jest martwa** — `nth-of-type(3)` wskazuje inną sekcję od czasu ADR-049 |
| T59 | **`R-TYPO-03` NIE ISTNIEJE W TYM REPOZYTORIUM** — 0 trafień przy kontroli pozytywnej `R-AKCENT` 28; zlecenie żąda zieleni rzeczy, której nie ma |
| T60 | **po przeszczepie interlinii prozy nic jej nie pilnuje** — 1 trafienie w `e2e/` i to liczące wiersze, nie asertujące |
| T61 | **2 px przepełnienia po zmianie rozmiaru okna** — przy świeżym wejściu 22/22 czyste, więc stan nieosiągalny dla odwiedzającej |
| T62 | **`meta-description` nie istnieje** — jedyny z trzech braków SEO, który przeżyje wdrożenie |
| T63 | **karta React Bits obiecuje „parallax hero", którego NIE MA** — trzy niezależne drogi pomiaru, kontrola pozytywna w każdej; zakres 3% u nas jest wyborem, nie miarą wzorca |
| T64 | **siatka bento zmierzona i niewdrożona** — 2 kolumny 403/605, odstęp 16, pierwsza karta `span 2`; brakuje decyzji, KTÓRA sekcja ma nią zostać |
| T65 | **rodzina wejścia „słowo po słowie" zmierzona i niewdrożona** — 0,15 → 1 w 67 ms na wyraz; wdrożenie to robota treściowa w trzech językach, nie stylistyczna |
| T66 | **wzorzec finalny jest kolażem dwukolumnowym** — rozmiary pisma i szerokości kontenera z niego nieprzenośne; barwy i proporcje TAK |
| T67 | **stosunek H2 : H1 wzorca 0,80 wobec naszego 0,50** — różnica konstrukcyjna, wymaga przeliczenia całej skali ADR-044 |
| T68 | **wzorzec jest stroną JASNĄ (70,9 % jasnych pikseli), nasza połowiczna (47,4 %)** — dokładna odwrotność mechanizmu tonów; największa różnica strukturalna |

---

## 16. Skorowidz ADR — **064 dopisany 2026-09-06** (`WWW/090` krok 3 v2)

Katalog `docs/adr/` — **liczby plików nie wpisuję**, bo to pole samostarzejące
się przy każdym nowym ADR: `ls docs/adr/*.md | wc -l`. **Wiążąca jest treść
pliku** — poniżej wyłącznie tytuły, żeby dało się trafić do właściwego bez
zgadywania.

⚠ **TABELA NIŻEJ KOŃCZY SIĘ NA 031 I JEST TO ZNANA NIEPEŁNOŚĆ, NIE KOMPLET.**
Numery **032–065 w niej nie stoją** — istnieją jako pliki i są wiążące,
a skorowidz ich nie wymienia. Zapisuję to w nagłówku, zamiast pozwolić, żeby
tabela udawała pełną listę: to ta sama klasa co „zły podzbiór nazwany
zbiorem”, tylko w skorowidzu ADR-ów zamiast w rejestrze. **Pełną listę bierze
się z katalogu, nie stąd:** `ls docs/adr/`. **NIE UZUPEŁNIAM TRZYDZIESTU
WIERSZY — zakaz 8**; `WWW/088` obejmuje cztery kroki o miarach wizualnych,
a uzupełnienie skorowidza jest osobną robotą redakcyjną.

Najświeższe, dopisane ręcznie, żeby nie zginęły do czasu uzupełnienia tabeli:
**052** anatomia wzorca · **053** batch A2 · **054** batch A3 · **055** siatka
2×3 i batch A4 · **056** kadr szeroki, duet K3, batch A5 · **057** podstrona
`/cennik` · **058** `/funkcje` i podstrony filarowe · **059** pas piksela ·
**060** pas pustki · **061** ludzie do slotów · **062** przeszczep miar
React Bits SaaS · **063** batch widoczny: hero i ruch React Bits · **064** **główna 1:1 z kolażu: wzorzec-kolumna, skala i klamra** · **065** wzorzec finalny: pomiar z pikseli i tokeny. ⚠ 064 powstał PO 065 — zlecenie domknęło lukę numeracji, więc kolejność numerów nie jest kolejnością czasu.
